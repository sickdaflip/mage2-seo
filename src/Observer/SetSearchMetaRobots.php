...
/**
 * Set NOINDEX,FOLLOW for search results
 */
class SetSearchMetaRobots implements ObserverInterface
{
    ...
    {
        try {
            if (!$this->seoHelper->getConfig('flipdev_seo/settings/noindexparamssearch')) {
                return;
            }

            $this->pageConfig->setRobots('NOINDEX,FOLLOW');
            
            $this->logger->debug('FlipDev_Seo: Set NOINDEX for search results');
        } catch (\Exception $e) {
            $this->logger->error('FlipDev_Seo: Search robots failed', [
